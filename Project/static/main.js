...
const autoLogin = async () => {
  ...
  try {
    if (data['access_token']) {
      ...
      // Handle the case when 'access_token' is not present in the 'data' dictionary.
      // For example, display an error message or redirect the user to the login page.
    }
  } catch (error) {
    console.log(`Error: ${error}`);
    return;
    // Handle any other potential errors that may occur during the process.
...
const onLogout = async () => {
  ...
  if (data.result) {
    ...
    // Redirect the user to the index.html page after successful logout.
    window.location.href = "/";
  } else {
    console.log("로그아웃 실패");
    // Handle the case when logout fails, for example, display an error message to the user.
  }
}

// Attach the onLogout function to the logout link
